...
static Error x86_fixup_instruction(AsmParser& parser, BaseInst& inst, Operand_* operands, uint32_t& count) noexcept {
  ...
  if (inst_id >= kX86AliasStart) {
    // Local fix: Assembler is not derived from x86::Emitter. Build the
    // architecture-dependent operands without an invalid polymorphic downcast.
    BaseEmitter* emitter = parser._emitter;
    const x86::Mem address_si = emitter->is_64bit() ? x86::ptr(x86::rsi) : x86::ptr(x86::esi);
    const x86::Mem address_di = emitter->is_64bit() ? x86::ptr(x86::rdi) : x86::ptr(x86::edi);
    uint32_t mem_size = 0;
    bool is_str = false;
    ...
    if (is_str) {
      if (count == 0) {
        ...
        count = 2;
        switch (inst_id) {
          case x86::Inst::kIdCmps: operands[0] = address_si; operands[1] = address_di; break;
          case x86::Inst::kIdMovs: operands[0] = address_di; operands[1] = address_si; break;
          case x86::Inst::kIdLods:
          case x86::Inst::kIdScas: operands[0] = Reg(reg_signature, x86::Gp::kIdAx); operands[1] = address_di; break;
          case x86::Inst::kIdStos: operands[0] = address_di; operands[1] = Reg(reg_signature, x86::Gp::kIdAx); break;
        }
      }
